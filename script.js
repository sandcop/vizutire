/* ══════════════════════════════════════════════
   VIZUtire — script.js
   GSAP animations · i18n · canvas · counters
   ══════════════════════════════════════════════ */
...
const T = {
  es: {
    ...
    'sis.sb1l':'ESTADO','sis.sb1v':'OPERATIVO',
    'sis.sb2l':'PROTOCOLO','sis.sb2v':'ACTIVO',
    'sis.sb3l':'MODO','sis.sb3v':'DIAGNÓSTICO',
    ...
    'val.c1':'Alta efectividad en detección estructural temprana',
    'val.c2':'Operaciones de clase mundial — Norte de Chile',
    'val.c3':'Flotas CAEX de 400 ton bajo operación continua',
    'val.c4':'Condiciones extremas: temperatura, carga y terreno',
    'val.c5':'Reducción comprobada de costos operacionales',
    ...
    'faq.q1':'¿Cómo funciona el diagnóstico in situ?',
    'faq.a1':'Utilizamos ultrasonido de alta frecuencia (2.4 MHz) aplicado directamente sobre el neumático montado. El proceso no requiere desmontaje ni interrupción de operaciones. Las señales capturan el estado interno de cada capa estructural y son procesadas por nuestro modelo físico-matemático en tiempo real.',
    'faq.q2':'¿Qué tipo de neumáticos OTR son compatibles?',
    'faq.a2':'VIZUtire es compatible con neumáticos OTR de gran minería desde 49" a 63", incluyendo los utilizados en camiones CAEX de 220 a 400+ toneladas (Caterpillar 793/795, Komatsu 830/930, Liebherr T282). El modelo ha sido validado en operaciones del norte de Chile.',
    'faq.q3':'¿Cuánto tiempo tarda el proceso completo?',
    'faq.a3':'La captura in situ por neumático demora entre 20 y 40 minutos. El diagnóstico completo con reporte de criticidad se entrega en menos de 48 horas desde la inspección. Para flotas completas, coordinamos ciclos de inspección programados que minimizan el impacto en la disponibilidad de equipos.',
    'faq.q4':'¿Es necesario detener la operación?',
    'faq.a4':'No. El diagnóstico se realiza con el equipo en zona de mantenimiento durante ventanas de servicio programadas, sin impacto en la continuidad operacional. El proceso es no destructivo e in situ: el neumático no requiere ser removido del vehículo.',
    'faq.q5':'¿Cómo se integra con nuestros sistemas de gestión?',
    'faq.a5':'Los reportes se entregan en formato digital estructurado (PDF + datos) compatible con los principales sistemas CMMS. El plan Enterprise incluye integración API directa con plataformas de gestión de activos. También ofrecemos dashboard operacional propio para seguimiento continuo del estado de flota.',

    'cta.eyebrow':'Diagnóstico OTR · Antofagasta, Chile',
    'cta.h1':'Reduce el riesgo estructural',
    'cta.h2':'antes del evento.',
  ...
  en: {
    'nav.sistema':'System','nav.resultados':'Results',
    'nav.testimonios':'Clients','nav.faq':'FAQ',
    'nav.pricing':'Plans','nav.cta':'Technical Assessment','ft.contact':'Contact',

    'hero.label':'Structural Diagnostics · OTR Mining',
    'hero.h1a':'Predictive diagnostics',
    'hero.h1b':'for structural failures',
    'hero.h1c':'in OTR tires.',
    ...
    'sis.sb1l':'STATUS','sis.sb1v':'OPERATIONAL',
    'sis.sb2l':'PROTOCOL','sis.sb2v':'ACTIVE',
    'sis.sb3l':'MODE','sis.sb3v':'DIAGNOSTIC',
    'sis.sbr':'VIZUtire v2.4 · OTR STRUCTURAL ENGINE',
    ...
    'val.c1':'High effectiveness in early structural detection',
    'val.c2':'World-class operations — Northern Chile',
    'val.c3':'400-ton CAEX fleets under continuous operation',
    'val.c4':'Extreme conditions: temperature, load and terrain',
    'val.c5':'Proven reduction of operational costs',
    ...
    'faq.q1':'How does the in-situ diagnosis work?',
    'faq.a1':'We use high-frequency ultrasound (2.4 MHz) applied directly to the mounted tire. The process requires no dismounting or operational interruption. Signals capture the internal state of each structural layer and are processed in real time by our physico-mathematical model.',
    'faq.q2':'What types of OTR tires are compatible?',
    'faq.a2':'VIZUtire is compatible with large mining OTR tires from 49" to 63", including those used in CAEX haul trucks from 220 to 400+ tons (Caterpillar 793/795, Komatsu 830/930, Liebherr T282). The model has been validated in Northern Chile operations.',
    'faq.q3':'How long does the complete process take?',
    'faq.a3':'In-situ capture per tire takes 20 to 40 minutes. The full diagnosis with criticality report is delivered in less than 48 hours from inspection. For full fleets, we coordinate scheduled inspection cycles that minimize impact on equipment availability.',
    'faq.q4':'Is it necessary to stop operations?',
    'faq.a4':'No. The diagnosis is performed with the equipment in the maintenance area during scheduled service windows, with no impact on operational continuity. The process is non-destructive and in-situ: the tire does not need to be removed from the vehicle.',
    'faq.q5':'How does it integrate with our management systems?',
    'faq.a5':'Reports are delivered in structured digital format (PDF + data) compatible with major CMMS systems. The Enterprise plan includes direct API integration with asset management platforms. We also offer our own operational dashboard for continuous fleet status monitoring.',

    'cta.eyebrow':'OTR Diagnostics · Antofagasta, Chile',
    'cta.h1':'Reduce structural risk',
    'cta.h2':'before the event.',
...
(function initCanvas() {
  ...
  class Pt {
    ...
    tick() {
      ...
      this.y += this.vy;
      this.life--;
      if (this.life <= 0 || this.x < 0 || this.x > cv.width || this.y < 0 || this.y > cv.height)
        this.reset();
    }
    draw() {
      ...
      ctx.fillStyle = `rgba(225,6,0,${this.a * fade * 0.5})`;
      ctx.fill();
    }
  }
    ...
    for (let i = 0; i < pts.length; i++) {
      for (let j = i + 1; j < pts.length; j++) {
        ...
        if (d < 110) {
          ...
          ctx.strokeStyle = `rgba(225,6,0,${0.06 * (1 - d / 110)})`;
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      }
  ...
  resize();
  pts = Array.from({ length: 55 }, () => new Pt());
  loop();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  heroTl
    .to('.hero-tag',   { opacity:1, duration:0.6, ease:'power3.out' })
    .to('.hero h1',    { opacity:1, y:0, duration:0.9, ease:'power4.out' }, '-=0.3')
    ...
    .to('.hero-trust', { opacity:1, y:0, duration:0.5, ease:'power3.out' }, '-=0.2')
    .to('.hero-visual',{ opacity:1, x:0, duration:1,   ease:'power4.out' }, '-=0.8');
  ...
  document.querySelectorAll('.reveal').forEach(el => {
    const delay = parseFloat(
      getComputedStyle(el).getPropertyValue('transition-delay') || '0'
    );
    gsap.from(el, {
      opacity: 0,
      y: 36,
  ...
  ['.testi-grid', '.pricing-grid', '.dashboard', '.prob-stats'].forEach(sel => {
...
function initCounters() {
  const counterEls = document.querySelectorAll('[data-cnt]');
  ...
  const runCounter = (el) => {
    ...
    function tick(now) {
      ...
      el.textContent = pfx + Math.round(e * end) + sfx;
...
/* ── HERO FALLBACK (no GSAP) ── */
function initHeroFallback() {
  const els = ['.hero-tag','.hero h1','.hero-desc','.hero-btns','.hero-stats','.hero-trust','.hero-visual'];
  els.forEach((sel, i) => {
